...
/**
 * BetterPIDController implements a PID control loop. This class is stolen from the 2020 WPILib beta software. I'd rather
 * use this new controller than the current one because I don't want to write thread-safe code.
 */
@SuppressWarnings("PMD.TooManyFields")
public class BetterPIDController extends SendableBase {
    ...
    public void setSetpoint(double setpoint) {
        if (m_maximumInput > m_minimumInput) {
            m_setpoint = limit(setpoint, m_minimumInput, m_maximumInput);
        } else {
            m_setpoint = setpoint;
        }
    }
    ...
    public double calculate(double measurement) {
        ...
        if (m_Ki != 0) {
            m_totalError = limit(m_totalError + m_positionError * m_period,
    ...
    private void setInputRange(double minimumInput, double maximumInput) {
        ...
        if (m_maximumInput > m_minimumInput) {
            m_setpoint = limit(m_setpoint, m_minimumInput, m_maximumInput);
        }
    }

    private double limit(double value, double low, double high) {
